  ...
  return (
    <div className="grid gap-4 lg:grid-cols-5">
      <section className="panel-shell lg:col-span-2">
        <h3 className="text-xl font-semibold text-slate-900">Blood Finder Filters</h3>
      ...
      <section className="panel-shell lg:col-span-3">
        <h3 className="text-xl font-semibold text-slate-900">Matching Sources</h3>
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {results.slice(0, 6).map((item) => (
            <article key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <p className="mt-1 text-xs text-blue-600">{item.phone}</p>
            </article>
          ))}
        </div>
