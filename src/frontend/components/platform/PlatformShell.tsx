...
export function PlatformShell({ children }: PlatformShellProps) {
  ...
  const [sessionType] = useState(
    () => (typeof window !== "undefined" ? localStorage.getItem("lifeline-session") : null) ?? "guest",
  );
  ...
  return (
    <div className="min-h-screen bg-command px-4 py-4 md:px-6">
      <div className="mx-auto max-w-screen-2xl">
        <section className="panel-shell relative overflow-hidden px-5 py-5 md:px-7 md:py-6">
          <div className="section-divider" />
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">LifeLine Platform</p>
              <h1 className="text-2xl font-semibold text-slate-900">{title}</h1>
              <p className="mt-1 text-sm text-slate-600">Emergency-ready dashboard aligned with your Figma frontend direction.</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="status-chip status-chip-live">AI Active</span>
              <span className="status-chip status-chip-live">Network Synced</span>
            </div>
          </div>
        </section>
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[260px_1fr_300px]">
          <aside className="panel-shell sticky top-24 flex h-[calc(100vh-7rem)] flex-col">
            <Link href="/" className="mb-6 block border-b border-slate-200 pb-4">
              ...
              <h2 className="text-xl font-semibold text-slate-900">Response Console</h2>
            </Link>
          ...
          <aside className="panel-shell sticky top-24 h-[calc(100vh-7rem)]">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-xs text-slate-500">Response Teams</p>
                <p className="mt-1 font-medium text-slate-900">Ambulance, Police, Fire monitored</p>
              </div>
            </div>
          </aside>
        </div>
