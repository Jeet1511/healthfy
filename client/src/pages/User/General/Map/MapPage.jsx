...
export default function MapPage() {
  ...
  const handleSavePlace = async () => { if (!selected || isGuest) return; await savePlace(selected); };
  const handleConfirmBooking = async (booking) => { if (isGuest) return; await addBooking(booking); setBookingOpen(false); };
      ...
      <AnimatePresence>
        {selected ? (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
            className="card interactive" style={{ marginTop: "1.5rem", borderLeft: "4px solid rgba(59, 130, 246, 0.8)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
            <div>
            ...
            <div className="action-buttons">
              <a className="action-pill btn-primary" href={`tel:${selected.phone}`} style={{ padding: "0.8rem 1.5rem", display: "inline-flex", gap: "8px", alignItems: "center" }}><Phone size={18} /> Call Direct</a>
              <a className="action-pill" href={`https://www.google.com/maps?q=${selected.latitude},${selected.longitude}`} target="_blank" rel="noopener noreferrer"
                style={{ padding: "0.8rem 1.5rem", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", display: "inline-flex", gap: "8px", alignItems: "center" }}><Navigation size={18} /> Navigate</a>
              <button type="button" className="action-pill" onClick={() => handleSavePlace().catch(() => undefined)} disabled={isGuest}
                style={{ padding: "0.8rem 1.5rem", display: "inline-flex", gap: "8px", alignItems: "center" }}><BookmarkPlus size={18} /> Save place</button>
              {canBook ? (<button type="button" className="action-pill btn-primary" disabled={isGuest} onClick={() => setBookingOpen(true)}
                style={{ padding: "0.8rem 1.5rem", display: "inline-flex", gap: "8px", alignItems: "center" }}><HeartPulse size={18} /> Book appointment</button>) : null}
            </div>
            {isGuest ? <p className="subtle" style={{ width: "100%", margin: 0 }}>Sign in to save places and bookings.</p> : null}
          </motion.section>
        ) : null}
      </AnimatePresence>
      <BookingModal open={bookingOpen} provider={selected} onClose={() => setBookingOpen(false)} onConfirm={handleConfirmBooking} disabled={isGuest} />
    </>
  );
}
