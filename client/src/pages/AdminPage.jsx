...
export default function AdminPage() {
  ...
  async function handleCreateProvider(event) {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Unable to create provider.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function activateProvider(providerId) {
    ...
    try {
      await apiClient.post("/admin/activate", { providerId }, tokenConfig(token));
      setMessage("Provider activated.");
      await fetchProviders();
    } catch (requestError) {
      setError(requestError.response?.data?.message || "Unable to activate provider.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function toggleProvider(providerId, isEnabled) {
    ...
    try {
      await apiClient.post("/admin/toggle-enabled", { providerId, isEnabled }, tokenConfig(token));
      setMessage(`Provider ${isEnabled ? "enabled" : "disabled"}.`);
      await fetchProviders();
    } catch (requestError) {
      setError(requestError.response?.data?.message || "Unable to update provider.");
    } finally {
      setLoading(false);
    }
  }
                ...
                <tbody>
                  {providers.map((provider) => (
                    <tr key={provider.id}>
                      ...
                      <td>{provider.maskedApiKey}</td>
                      <td>
                        <span className={provider.isActive ? "type-chip tone-safe" : "type-chip tone-warning"}>
                          {provider.isActive ? "Active" : "Inactive"}
                        </span>
                        <span className={provider.isEnabled ? "type-chip tone-safe" : "type-chip tone-danger"}>
                          {provider.isEnabled ? "Enabled" : "Disabled"}
                        </span>
                      </td>
                      <td className="admin-actions">
                        <button type="button" onClick={() => activateProvider(provider.id)} disabled={loading || !provider.isEnabled}>
                          Activate
                        </button>
                        <button
                          type="button"
